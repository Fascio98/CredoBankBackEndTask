...
public class GetUsersNegativeCases422ResponseCodeApiTests {
    ...
    public void beforeClass() {
        stubs.resetWiremock();
        for(int i = 0; i < negativeCasesData.negativeStubs422ResponseCode.length; i++) {
            stubs.createStub(negativeCasesData.negativeStubs422ResponseCode[i]);
        }
    }

    @Test(dataProvider = "negative422CasesData", dataProviderClass = TestDataProviders.class)
    public void testNegative422Cases(String testName, Map<String, String> queryParams, String expectedResponse, int expectedStatusCode) {
        Response response;
